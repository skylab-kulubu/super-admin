...
import { BizbizeEvent } from '@/types';
import Button from '@/components/ui/Button';
import { PencilSquareIcon, PhotoIcon, CheckCircleIcon, XCircleIcon } from '@heroicons/react/24/outline';

interface BizbizeEventTableProps {
  events: BizbizeEvent[];
  onEditEvent: (event: BizbizeEvent) => void;
  onManagePhotos: (event: BizbizeEvent) => void;
}

const BizbizeEventTable: React.FC<BizbizeEventTableProps> = ({ events, onEditEvent, onManagePhotos }) => {
  return (
    <div className="overflow-x-auto bg-white dark:bg-gray-800 shadow-md rounded-lg">
      <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
        <thead className="bg-gray-50 dark:bg-gray-700">
          <tr>
            <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Misafir</th>
        ...
        <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
          ...
          {events.map((event) => (
            <tr key={event.id} className="hover:bg-gray-100 dark:hover:bg-gray-700">
              <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-700 dark:text-gray-200">{event.guestName}</td>
...
export default BizbizeEventTable;
