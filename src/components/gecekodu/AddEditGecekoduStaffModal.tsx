...
import { GecekoduStaff, AddGecekoduStaffPayload, UpdateGecekoduStaffPayload, ImageUploadResponseData, PhotoId, ApiResponse } from '@/types';

interface AddEditGecekoduStaffModalProps {
  isOpen: boolean;
  onClose: () => void;
  onStaffSaved: () => void;
  staffToEdit?: GecekoduStaff | null;
}
...
const AddEditGecekoduStaffModal: React.FC<AddEditGecekoduStaffModalProps> = ({ isOpen, onClose, onStaffSaved, staffToEdit }) => {
  ...
  const processPhotoUpload = async (file: File): Promise<number | null> => {
    try {
      ...
      const photoRes = await api.post<ApiResponse<PhotoId>>('/photos/addPhoto', { photoUrl: imageUrl, tenant: "GECEKODU" });
  ...
  const onSubmit: SubmitHandler<StaffFormInputs> = async (data) => {
    ...
    try {
      if (isEditMode && staffToEdit) {
        const payload: UpdateGecekoduStaffPayload = {
          id: staffToEdit.id,
          ...data,
        };
        ...
        const payload: AddGecekoduStaffPayload = {
          ...data,
      ...
      console.error("Save GeceKodu staff error:", error);
      ...
      <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-xl w-full max-w-lg my-8">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-xl font-semibold text-gray-800 dark:text-white">
            {isEditMode ? 'GeceKodu Ekip Üyesini Düzenle' : 'Yeni GeceKodu Ekip Üyesi Ekle'}
          </h2>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          <div className="flex flex-col items-center space-y-2">
            ...
            <input type="file" accept="image/*" onChange={handleFileChange} ref={fileInputRef} className="hidden" id="gecekodu-staff-photo-upload"/>
...
export default AddEditGecekoduStaffModal;
