...
import { GecekoduEvent, BizbizeEventPhoto as GecekoduEventPhoto, ImageUploadResponseData, PhotoId, ApiResponse } from '@/types';
import { XMarkIcon, CloudArrowUpIcon } from '@heroicons/react/24/outline';
import Image from 'next/image';

interface ManageGecekoduEventPhotosModalProps {
  isOpen: boolean;
  onClose: () => void;
  event: GecekoduEvent | null; 
  onPhotosUpdated: () => void;
}

const ManageGecekoduEventPhotosModal: React.FC<ManageGecekoduEventPhotosModalProps> = ({ isOpen, onClose, event, onPhotosUpdated }) => {
  const [isLoading, setIsLoading] = useState(false);
  const [currentPhotos, setCurrentPhotos] = useState<GecekoduEventPhoto[]>([]);
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const photoMetadataPayload = { photoUrl: imageUrl, tenant: "GECEKODU" };
      ...
      const newPhoto: GecekoduEventPhoto = { 
        id: photoId, 
        photoUrl: imageUrl, 
        tenant: "GECEKODU" 
      };
      ...
      console.error("Add photo to GeceKodu event error:", error);
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50 overflow-y-auto">
      <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-xl w-full max-w-2xl my-8">
        ...
        <div className="mt-6">
          <input type="file" accept="image/*" onChange={handleFileChange} ref={fileInputRef} className="hidden" id="gecekodu-event-photo-upload"/>
...
export default ManageGecekoduEventPhotosModal;
