...
import { AgcEvent, BizbizeEventPhoto as AgcEventPhoto, ImageUploadResponseData, PhotoId, ApiResponse } from '@/types';
import { XMarkIcon, CloudArrowUpIcon } from '@heroicons/react/24/outline';
import Image from 'next/image';

interface ManageAgcEventPhotosModalProps {
  isOpen: boolean;
  onClose: () => void;
  event: AgcEvent | null; 
...
const ManageAgcEventPhotosModal: React.FC<ManageAgcEventPhotosModalProps> = ({ isOpen, onClose, event, onPhotosUpdated }) => {
  const [isLoading, setIsLoading] = useState(false);
  const [currentPhotos, setCurrentPhotos] = useState<AgcEventPhoto[]>([]);
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const photoMetadataPayload = { photoUrl: imageUrl, tenant: "AGC" };
      ...
      console.error("Add photo to AGC event error:", error);
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50 overflow-y-auto">
      <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-xl w-full max-w-2xl my-8">
        ...
        <div className="mt-6">
          <input type="file" accept="image/*" onChange={handleFileChange} ref={fileInputRef} className="hidden" id="agc-event-photo-upload"/>
...
export default ManageAgcEventPhotosModal;
