...
import { AgcEvent, AddAgcEventPayload, UpdateAgcEventPayload } from '@/types'; // AGC types
import { formatDateTimeForApi, parseApiDateTimeToInput } from '@/lib/utils';

interface AddEditAgcEventModalProps {
  isOpen: boolean;
  onClose: () => void;
  onEventSaved: () => void;
  eventToEdit?: AgcEvent | null;
}
...
const AddEditAgcEventModal: React.FC<AddEditAgcEventModalProps> = ({ isOpen, onClose, onEventSaved, eventToEdit }) => {
  ...
  useEffect(() => {
    if (eventToEdit) {
      ...
      reset({ 
        ...
        linkedin: '',
        isActive: true,
        type: 'EVENT', // Default type for AGC, adjust if needed
        description: '',
        formUrl: '',
      });
    }
  ...
  const onSubmit: SubmitHandler<EventFormInputs> = async (data) => {
    ...
    try {
      if (isEditMode && eventToEdit) {
        const payload: UpdateAgcEventPayload = {
          id: eventToEdit.id,
          title: data.title,
        ...
        await api.post('/events/updateEvent', payload); 
        toast.success('Etkinlik başarıyla güncellendi!');
      } else {
        const payload: AddAgcEventPayload = {
          title: data.title,
          description: data.description,
          date: formattedDate,
      ...
      console.error("Save AGC event error:", error);
      ...
      <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-xl w-full max-w-lg my-8">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-xl font-semibold text-gray-800 dark:text-white">
            {isEditMode ? 'AGC Etkinliğini Düzenle' : 'Yeni AGC Etkinliği Ekle'}
          </h2>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div className="flex items-center">
            ...
            />
            <label htmlFor="isActiveAGCEvent" className="text-sm text-gray-700 dark:text-gray-300">Aktif Etkinlik</label>
          </div>
...
export default AddEditAgcEventModal;
