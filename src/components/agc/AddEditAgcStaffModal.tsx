...
import { AgcStaff, AddAgcStaffPayload, UpdateAgcStaffPayload, ImageUploadResponseData, PhotoId, ApiResponse } from '@/types';

interface AddEditAgcStaffModalProps {
  isOpen: boolean;
  onClose: () => void;
  onStaffSaved: () => void;
  staffToEdit?: AgcStaff | null;
}
...
const AddEditAgcStaffModal: React.FC<AddEditAgcStaffModalProps> = ({ isOpen, onClose, onStaffSaved, staffToEdit }) => {
  ...
  const processPhotoUpload = async (file: File): Promise<number | null> => {
    try {
      ...
      const photoRes = await api.post<ApiResponse<PhotoId>>('/photos/addPhoto', { photoUrl: imageUrl, tenant: "AGC" });
  ...
  const onSubmit: SubmitHandler<StaffFormInputs> = async (data) => {
    ...
    try {
      if (isEditMode && staffToEdit) {
        const payload: UpdateAgcStaffPayload = {
          id: staffToEdit.id,
          ...data,
        };
        ...
        const payload: AddAgcStaffPayload = {
          ...data,
      ...
      console.error("Save AGC staff error:", error);
      ...
      <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-xl w-full max-w-lg my-8">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-xl font-semibold text-gray-800 dark:text-white">
            {isEditMode ? 'AGC Ekip Üyesini Düzenle' : 'Yeni AGC Ekip Üyesi Ekle'}
          </h2>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          <div className="flex flex-col items-center space-y-2">
            ...
            <input type="file" accept="image/*" onChange={handleFileChange} ref={fileInputRef} className="hidden" id="agc-staff-photo-upload"/>
...
export default AddEditAgcStaffModal;
